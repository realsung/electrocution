...
export default class DangerousFunctionsJSCheck {
  ...
  match(astNode, astHelper, scope) {
    ...
    const electronMethods = ['executeJavascript', 'insertCSS'];

    const evalLikemethods = ['eval', 'Function', 'setTimeout', 'setInterval', 'setImmediate'];
    ...
    if (evalLikemethods.includes(astNode.callee.name) || astNode.callee.property && evalLikemethods.includes(astNode.callee.property.name)) {
      ...
      if (astNode.arguments[0].type === "Identifier") {
        ...
        if (target === null) // we could not resolve the variable, let the user check
          shouldReport = true;else if (target && (target.type === "BinaryExpression" || target.type.includes(astHelper.StringLiteral))) // check if we are facing a string concatenation, a plain string or template literal (e.g. ${var})
          shouldReport = true;else // it's probably of little interest
          shouldReport = false;
      }
    }

    if (shouldReport) return [{ line: astNode.loc.start.line, column: astNode.loc.start.column, id: this.id, description: this.description, shortenedURL: this.shortenedURL, severity: severity.MEDIUM, confidence: confidence.CERTAIN, manualReview: true }];
  }
}
//# sourceMappingURL=DangerousFunctionsJSCheck.js.map