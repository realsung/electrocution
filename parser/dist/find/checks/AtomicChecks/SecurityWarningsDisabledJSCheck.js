  ...
  match(astNode) {
    ...
    if (astNode.left.object && astNode.left.object.property) {
      if (astNode.left.object.property.name === "env" || astNode.left.object.property.name === "webContents") {
        if (astNode.left.property && astNode.left.property.value && astNode.left.property.value.toString().toUpperCase() === "ELECTRON_DISABLE_SECURITY_WARNINGS" && astNode.right.value) return [{ line: astNode.loc.start.line, column: astNode.loc.start.column, id: this.id, description: this.description, shortenedURL: this.shortenedURL, severity: severity.INFORMATIONAL, confidence: confidence.CERTAIN, manualReview: false }];
      }
    }
...
//# sourceMappingURL=SecurityWarningsDisabledJSCheck.js.map