...
export default class AvailableSecurityFixesGlobalCheck {
        ...
        if (issue.properties.versionNumber !== latestRelease) {
          ...
          if (confidenceIfSecurityFixAvailable) {
            if (issue.manualReview) // found in devDependencies
              otherIssues.push({ file: versionCheckIssues[0].file, location: { line: 0, column: 0 }, id: this.id, description: this.description.SECURITY_ISSUES, shortenedURL: this.shortenedURL, severity: severity.INFORMATIONAL, confidence: confidenceIfSecurityFixAvailable, manualReview: issue.manualReview });else // found in dependencies
  ...
  async checkSecurityFixes(version, releases) {
    ...
    for (let release of followingReleases) {
      for (let regex of this.releaseNoteSecurityFixRegex) if (regex.test(release.body)) securityFixes = confidence.CERTAIN;
    }
  ...
  async updateReleasesList(output) {
    ...
    if (!shouldUpdate) {
      ...
      if (releaseFile.length > 0) {
        // file exists, we should check the etag
        try {
        ...
        if (localEtag === remoteEtag) shouldUpdate = false;else {
          shouldUpdate = true;
          //remove the old releases.json file(s)
          try {
            for (let file of releaseFile.values()) fs.unlinkSync(path.join(this.releasesFilePath, file));
          } catch (e) {
    ...
    if (shouldUpdate) {
      if (!output) console.log(chalk.green(`Fetching Electron's new releases, this may take a while...`));
      try {
      ...
      if (!output) console.log(chalk.green(`Updated releases list to ${latest}!`));
      return true;
    } else {
      if (!output) console.log(chalk.green(`Releases list is up to date.`));
    }
  }

}
//# sourceMappingURL=AvailableSecurityFixesGlobalCheck.js.map