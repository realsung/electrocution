...
class Finder {
  constructor(customScan, excludeFromScan, electronUpgrade) {
    ...
    if (customScan && customScan.length > 0) {
      ...
      if (!customScan.every(r => checksNames.includes(r))) {
        ...
        for (var i = this._enabled_checks.length - 1; i >= 0; i--) if (!customScan.includes(this._enabled_checks[i].name.toLowerCase())) this._enabled_checks.splice(i, 1);
      }
    }
    ...
    if (excludeFromScan && excludeFromScan.length > 0) {
      ...
      if (!excludeFromScan.every(r => checksNames.includes(r))) {
        ...
        for (var i = this._enabled_checks.length - 1; i >= 0; i--) if (excludeFromScan.includes(this._enabled_checks[i].name.toLowerCase())) this._enabled_checks.splice(i, 1);
      }
    }
  ...
  get enabled_checks() {
    return this._enabled_checks;
  }

  get checks_by_type() {
    return this._checks_by_type;
  }
